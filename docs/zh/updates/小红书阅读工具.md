...
- 使用 **`nanobot run`** 启动网关时，若 `tools.chromeDebug.autoStartChrome` 为 true（默认），本程序会**自动启动一个 Chrome 进程**，并开启 `tools.chromeDebug.cdpPort`（默认 19327），供 read_rednote 等工具连接。
...
- 使用 **`nanobot agent`** 或 **`nanobot cron run`** 等命令时，**不会**自动启动 Chrome；read_rednote 仍会注册，但若要生效，需要：
  ...
  - 先通过 **`nanobot run`** 启动网关（此时会按配置自动启动 Chrome），再在其它终端用 agent/cron 等。
...
- **Chrome 启动**：`nanobot/utils/chrome_launcher.py` — `start_chrome_debug(port, user_data_dir, headless)`；在 `nanobot/cli/commands.py` 的 `run()` 中根据 `tools.chromeDebug.autoStartChrome` 调用，并在退出时 terminate 子进程。
